...
def main():
    color1 = "gray"
    color2 = "pink"
    print(f") Roll a ball using {color1}.\n")
    choice1 = input("long body or round body? ")
    # CAUTION: You must include the word "body" when checking!
    if choice1 == "Long body":
     print(") Roll the ball into an egg shape.\n")
    else:
     print(") Keep it as a ball.\n")
    print(f") Roll a smaller ball using {color1} for the head.\n")
    print(") Attach the head to one end of the body.\n")
    choice2 = input("long tail or short tail? ")
    if choice2 == "long tail":
     print(f") Roll a thin rope using {color2} and attach to the back.\n")
    else:
     print(f") Add a small bump using {color2} to the back.\n")
    print(f") Add four small legs to the bottom using {color1}.\n")
    print(") Add two dots for eyes and a tiny nose.\n")
